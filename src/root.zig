...
const Lua = struct {
    ...
    /// ```
    pub fn push(self: Lua, value: anytype) void {
    ...
    pub inline fn pop(self: Lua, comptime T: type) ?T {
        defer self.state.pop(1);
        return self.toValue(T, -1);
    }

    pub inline fn top(self: Lua) i32 {
    ...
    ///
    /// Returns: `?T` - The converted value, or `null` if conversion failed
    pub fn toValue(self: Lua, comptime T: type, index: i32) ?T {
        const type_info = @typeInfo(T);
        switch (type_info) {
            .bool => {
